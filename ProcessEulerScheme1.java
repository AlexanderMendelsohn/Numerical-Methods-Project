...
import net.finmath.stochastic.RandomVariableInterface;
	 ...
	 * @version 1.4
	 */
	public class ProcessEulerScheme1 extends AbstractProcess {

		public enum Scheme {
			EULER, PREDICTOR_CORRECTOR, JUMP_DIFFUSION
		};
		 ...
		 */
		public ProcessEulerScheme1(BrownianMotionInterface brownianMotion) {
			super(brownianMotion.getTimeDiscretization());
			this.brownianMotion = brownianMotion;
		}
		
		public ProcessEulerScheme1(BrownianMotionInterface brownianMotion, PointProcessInterface compoundPoissonProcess) {
			super(brownianMotion.getTimeDiscretization());
			this.brownianMotion = brownianMotion;
		...
		private void doPrecalculateProcess() {
			...
			if (scheme == Scheme.JUMP_DIFFUSION) {
			// add jump increment to realizations at next time step

			RandomVariableInterface jumpIncrement = compoundPoissonProcess.getProcessIncrements(timeIndex);

				if (jumpIncrement== null || jumpIncrement == null) continue;
				
				// Add drift adjustment
				//Jump_Diffusion is only devised for one-dimensional process. Hence the component index is 0
				currentState[0] = currentState[0].add(currentState[0].mult(jumpIncrement));
				
				//Jump_Diffusion is only devised for one-dimensional process. Hence the component index is 0
				discreteProcess[timeIndex][0] = applyStateSpaceTransform(0, currentState[0]);
		} // End if(scheme == Scheme.JUMP_DIFFUSION)
	...
	try {
		 ...
		 * no longer immutable, should only be used for EULER and JUMP_DIFFUSION
		 */
		 ...
		 */
		@Override
		public ProcessEulerScheme1 clone() {
			return new ProcessEulerScheme1(getBrownianMotion());
		}
		...
		public Object getCloneWithModifiedSeed(int seed) {
			return new ProcessEulerScheme1((BrownianMotionInterface)getBrownianMotion().getCloneWithModifiedSeed(seed));
		}
	}